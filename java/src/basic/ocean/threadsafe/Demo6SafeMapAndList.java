 ...
 */
public class Demo6SafeMapAndList {
    public static void main(String[] args) {
         ...
         * ConcurrentHashMap和Hashtable之间的区别
         那么Hashtable和ConcurrentHashMap之间的区别是什么，可以在多线程环境中使用，
         但一旦Hashtable的大小变得相当大的性能降低，因为迭代它必须被锁定更长的时间。
         ...
         ConcurrentHashMap旨在实现并发性能，提高性能，而通过使用synchronized Map应用包装器，
         可以同步自然不同步的HashMap。
         ...
         还有他们如何实现的线程安全。同步HashMap, Hashtable, HashSet, Vector, ArrayList
         相比他们并发的实现（比如：ConcurrentHashMap, CopyOnWriteArrayList, CopyOnWriteHashSet）会慢得多。
         **造成如此慢的主要原因是锁， 同步集合会把整个Map或List锁起来，而并发集合不会。**
         并发集合实现线程安全是通过使用先进的和成熟的技术像锁剥离。比如ConcurrentHashMap 会把整个Map 划分成几个片段，
         只对相关的几个片段上锁，同时允许多线程访问其他未上锁的片段。
         同样的，CopyOnWriteArrayList 允许多个线程以非同步的方式读，当有线程写的时候它会将整个List复制一个副本给它。
         如果在读多写少这种对并发集合有利的条件下使用并发集合，这会比使用同步集合更具有可伸缩性。
         顺便说一下，集合类是Java API的核心，所以我觉得明智而审慎地使用它们是一门艺术。
         这是我的个人经验,我通过使用ArrayList替换遗留代码里使用的Vector来提高性能。
         JDK 1.5引入一些很好的并发集合能高效地开发高容量,低延迟的Java应用程序。
         Synchronized Collections vs Concurrent Collections in Java
         同步集合类，Hashtable 和 Vector 还有同步集合包装类，Collections.synchronizedMap()
         和Collections.synchronizedList() 提供了一个基本的有条件的线程安全的Map和List的实现。
